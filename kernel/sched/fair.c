				    ...
				    struct task_struct *p)
{
	...
	/* Update root cfs_rq's estimated utilization */
	enqueued  = cfs_rq->avg.util_est.enqueued;
	enqueued -= min_t(unsigned int, enqueued, (_task_util_est(p) | UTIL_AVG_UNCHANGED));
	WRITE_ONCE(cfs_rq->avg.util_est.enqueued, enqueued);
...
static unsigned long cpu_util_without(int cpu, struct task_struct *p)
{
	...
	if (sched_feat(UTIL_EST)) {
		...
		if (unlikely(task_on_rq_queued(p) || current == p))
			lsub_positive(&estimated, (_task_util_est(p) | UTIL_AVG_UNCHANGED));
		...
		struct perf_domain *pd, struct task_struct *p, int prev_cpu)
{
	...
	for (; pd; pd = pd->next) {
		...
		for_each_cpu_and(cpu, perf_domain_span(pd), sched_domain_span(sd)) {
			if (!cpumask_test_cpu(cpu, &p->cpus_allowed) || is_reserved(cpu))
				continue;
