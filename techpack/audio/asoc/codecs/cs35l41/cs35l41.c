...
#define FW_RELOAD_DONE 0
#define FW_RELOAD_NEED 1
#define FW_RELOAD_PROC 2
...
static int cs35l41_force_reload_get(struct snd_kcontrol *kcontrol,
				      struct snd_ctl_elem_value *ucontrol);
static int cs35l41_force_reload_put(struct snd_kcontrol *kcontrol,
				      struct snd_ctl_elem_value *ucontrol);
static void cs35l41_fw_force_reload(struct snd_soc_component *component);
...
static const char *cs35l41_fast_switch_text[] = {
	...
	"cs35l41-delta-spk-handset.txt",
	"cs35l41-delta-rcv-music.txt",
	"cs35l41-delta-rcv-handsfree.txt",
	"cs35l41-delta-rcv-handset.txt",
	"cs35l41-revB2-delta-spk-music.txt",
	"cs35l41-revB2-delta-spk-handsfree.txt",
	"cs35l41-revB2-delta-spk-handset.txt",
	"cs35l41-revB2-delta-rcv-music.txt",
	"cs35l41-revB2-delta-rcv-handsfree.txt",
	"cs35l41-revB2-delta-rcv-handset.txt",
};
...
static const struct snd_kcontrol_new cs35l41_aud_controls[] = {
	...
	SOC_SINGLE_EXT("Force Reload", SND_SOC_NOPM, 0, 1, 0,
			cs35l41_force_reload_get, cs35l41_force_reload_put),
		...
		struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		...
		if (cs35l41->dsp.running) {
			...
			default:
				dev_err(cs35l41->dev,
					"Firmware status is invalid(0x%x)\n",
					fw_status);
				break;
			}
	...
	/*
	 * There should be something wrong with the firmware
	 * if mailbox failed.
	 * Mailbox failed on power up event, run reload immediately.
	 * Mailbox failed on power down event, will apply reloading
	 * on next power up event.
	 */
	if ((ret && event==SND_SOC_DAPM_POST_PMU
...
static void cs35l41_fw_reload_work(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct cs35l41_private *cs35l41 =
		container_of(dwork, struct cs35l41_private, reload_work);
	struct snd_soc_component *component = cs35l41->component;
	struct snd_soc_dapm_context *dapm
		= snd_soc_component_get_dapm(component);
	const char *prefix = component->name_prefix;

	int ret = 0;
	char fullname[80] = {0};
	const char *wname = "DSP1 Preloader";
	...
	w = dapm_find_widget(dapm, fullname, NULL);

	/* Mute to eliminate pops for firmware loading */
	regmap_update_bits(cs35l41->regmap,
			CS35L41_AMP_DIG_VOL_CTRL, 0x7ff << 3, 0x400 << 3);

	/* Power down AMP */
	ret = cs35l41_main_amp_event(w, NULL, SND_SOC_DAPM_POST_PMD);
	if(ret < 0)
		dev_err(cs35l41->dev,
				"Failed on main event PMD\n");

	/* Stop core */
	ret = cs35l41_dsp_power_ev(w, NULL, SND_SOC_DAPM_PRE_PMD);
	if(ret < 0)
		dev_err(cs35l41->dev,
				"Failed on power event PMD\n");

	/* Loading ...*/
	ret = cs35l41_dsp_power_ev(w, NULL, SND_SOC_DAPM_PRE_PMU);
	if(ret < 0)
		dev_err(cs35l41->dev,
				"Failed on power event PMU\n");


	/* Start core */
	ret = cs35l41_dsp_load_ev(w, NULL, SND_SOC_DAPM_POST_PMU);
	if(ret < 0)
		dev_err(cs35l41->dev,
				"Failed on load event PMU\n");

	/* Transfer delta tuning*/
	...
	/* Power up AMP*/
	ret = cs35l41_main_amp_event(w, NULL, SND_SOC_DAPM_POST_PMU);
	if(ret < 0)
		dev_err(cs35l41->dev,
...
static void cs35l41_fw_force_reload(struct snd_soc_component *component)
{
	...
	if(cs35l41->dsp.fw == WM_ADSP_FW_SPK_CALI
			|| cs35l41->dsp.fw == WM_ADSP_FW_SPK_DIAG)
		return;

	/* TODO: Is the core dead already ? */
...
static void cs35l41_pcm_shutdown(struct snd_pcm_substream *substream,
...
/*
* This patch only for the silet issue on 6.31 firmware
* The name bellow is vary with diffent product
* TODO: Remove this after issue fixed in firmware
*/

#define SPK_DAI_NAME "cs35l41.0-0040"
#define RCV_DAI_NAME "cs35l41.0-0041"
#define HANDSET_TUNING "cs35l41-delta-rcv-handset.txt"
...
static int cs35l41_component_probe(struct snd_soc_component *component)
{
	...
	regmap_write(cs35l41->regmap, CS35L41_BSTCVRT_VCTRL2, 0x5); /* Apply to B2 chip as well*/
				...
				struct cs35l41_platform_data *pdata)
{
	...
	cs35l41->rev = regid;
	if (regid != CS35L41LV_CHIP_ID) /* It should be B2 chip */
		cs35l41->pdata.fwname_use_revid = true;
	...
	INIT_DELAYED_WORK(&cs35l41->reload_work, cs35l41_fw_reload_work);
	cs35l41->reload_status = FW_RELOAD_DONE;
	mutex_init(&cs35l41->hb_lock);
	mutex_init(&cs35l41->reload_lock);
err:
