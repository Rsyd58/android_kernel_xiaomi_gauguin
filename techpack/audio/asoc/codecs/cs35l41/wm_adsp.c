			     ...
			     struct snd_ctl_elem_value *ucontrol)
{
	...
	adsp_info(dsp, "put cal_z=%d, cal_checksum=%d\n",
				    ...
				    struct snd_ctl_elem_value *ucontrol)
{
	...
	adsp_info(dsp, "put calib status=%d\n", dsp->cal_status);
				    ...
				    struct snd_ctl_elem_value *ucontrol)
{
	...
	adsp_info(dsp, "put calib checksum = %d\n", dsp->cal_chksum);
				   ...
				   int noffs, unsigned int *offs)
{
	...
	scratch = kmalloc(sizeof(*scratch), GFP_KERNEL | GFP_DMA);
	if (!scratch)
		return;

	for (i = 0; i < noffs; ++i) {
		ret = regmap_read(dsp->regmap, dsp->base + offs[i], scratch);
		if (ret) {
	...
	while (pos < firmware->size &&
		...
		case WMFW_HALO_YM_PACKED:
			...
			if (type == WMFW_HALO_PM_PACKED)
				burst_multiple = 20;
			else if ((type == WMFW_HALO_XM_PACKED) ||
...
int wm_adsp_write_ctl(struct wm_adsp *dsp, const char *name, void *buf,
		      size_t len)
{
	struct wm_coeff_ctl *ctl;
...
static int wm_halo_apply_calibration(struct wm_adsp *dsp)
{
    __be32 cal_z = cpu_to_be32(dsp->cal_z);
    __be32 ambient = cpu_to_be32(dsp->ambient);
    __be32 status = cpu_to_be32(dsp->cal_status);
    __be32 chksum = cpu_to_be32(dsp->cal_chksum);
    __be32 us_bypass = cpu_to_be32(1);

    switch(dsp->fw) {
        case WM_ADSP_FW_SPK_CALI:
        case WM_ADSP_FW_SPK_DIAG:
            adsp_info(dsp, "calibration set ambient=%d\n", dsp->ambient);
            wm_adsp_write_ctl(dsp, "CAL_AMBIENT", &ambient, sizeof(__be32));
            break;
        case WM_ADSP_FW_SPK_PROT:
            adsp_info(dsp, "write: ambient=%d, cal_r=%d, status=%d, chksum=%d\n",
                    dsp->ambient, dsp->cal_z, dsp->cal_status, dsp->cal_chksum);
            wm_adsp_write_ctl(dsp, "CAL_R", &cal_z, sizeof(__be32));
            wm_adsp_write_ctl(dsp, "CAL_STATUS", &status, sizeof(__be32));
            wm_adsp_write_ctl(dsp, "CAL_CHECKSUM", &chksum, sizeof(__be32));

            wm_adsp_read_ctl(dsp, "CAL_R", &cal_z, sizeof(__be32));
            wm_adsp_read_ctl(dsp, "CAL_STATUS", &status, sizeof(__be32));
            wm_adsp_read_ctl(dsp, "CAL_CHECKSUM", &chksum, sizeof(__be32));

            adsp_info(dsp, "read: cal_r=%d, status=%d, chksum=%d\n",
                    cpu_to_be32(cal_z), cpu_to_be32(status), cpu_to_be32(chksum));

            /* Set ultrasonic bypass = 1 for TDM solution*/
            wm_adsp_write_ctl(dsp, "ENABLE_FULL_US_BYPASS", &us_bypass, sizeof(__be32));
            wm_adsp_read_ctl(dsp, "ENABLE_FULL_US_BYPASS", &us_bypass, sizeof(__be32));
            adsp_info(dsp, "read us_bypass = %d\n", cpu_to_be32(us_bypass));
